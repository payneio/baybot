...
uint8_t I2C::ReadBytes(uint8_t register_number, uint8_t bufferSize,
                       uint16_t &position) {
  if (fd_ == -1) {
    ROS_ERROR("Device File not available. Aborting read");
    return (-1);
...
*/
uint8_t I2C::WriteByte(uint8_t address, uint8_t b1, uint8_t b2, uint8_t b3,
                       uint8_t b4) {
  if (fd_ == -1) {
  ...
  if (result != 2) {
    ROS_ERROR("Failed to write. I2C: 0x%x, register 0x%x, err: %d", i2caddr_,
              address, errno);
    return (-1);
  }

  ROS_INFO("Wrote. I2C: 0x%x, register: 0x%d, data: 0x%x", i2caddr_, address,
           b1);
  return (0);
}
